...
import { getMesaById, updateMesa } from '../../shared/api.js';
...
class EditMesaPage extends HTMLElement {
    async connectedCallback() {
        ...
        if (id) {
            await this.loadMesa(id);
        } else {
            toast('ID da mesa não encontrado!');
        ...
        this.querySelector('#form-mesa').addEventListener('submit', async (e) => {
            ...
            const loading = await showLoading('Atualizando mesa...');
            try {
                await updateMesa(id, dados);
                toast('Mesa atualizada com sucesso!', 'success');
                document.querySelector('ion-router').push('/mesa/list', 'back');
            } catch (err) {
                toast(err.message);
            } finally {
    ...
    async loadMesa(id) {
        ...
        try {
            ...
            form.querySelector('[name="qtd_cadeiras"]').value = mesa.qtd_cadeiras;
            form.querySelector('[name="status"]').value = mesa.status ? '1' : '0';
        } catch (err) {
            toast(err.message);
        } finally {
            await loading.dismiss();
